...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 17, 2024 at 01:04 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getfilmbytype` (IN `type` VARCHAR(3))  DETERMINISTIC BEGIN
SELECT f.filmnimetus, t.filmtype
FROM film f
INNER JOIN filmtype t ON f.filmtypeID=t.filmTypeID
WHERE t.filmtype= type;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `getfilmid` (IN `pikkus_val` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tervist` ()  DETERMINISTIC COMMENT 'The prodceudre' BEGIN
	SELECT 'hi world!';
END$$
...
(1, 'Pulp Fiction', 8, 1, 3, 1, 1, ''),
(2, 'Vabastatud Django', 1, 1, 2, 1, 2, ''),
(3, 'Ongi Karloson!', 4, 2, 1, 2, 2, ''),
(4, 'Roheline elevant', 7, 2, 2, 3, 1, ''),
(5, 'Kaardid, rahad, kaks püst', 4, 3, 4, 5, 2, '');
